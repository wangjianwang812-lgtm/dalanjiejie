...
with col_r:
    ...
    # 截断渲染：只显示前 300 注，解决页面卡顿
    preview = " ".join(st.session_state.res_list[:300])
    if len(st.session_state.res_list) > 300: preview += "\n\n... (仅显示前300注)"
    ...
    if st.session_state.res_list:
        ...
        <button id="copyBtn" class="unified-btn" onclick="
            ...
            document.getElementById('copyBtn').innerText = '✅ 已复制全量';
