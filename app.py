...
st.markdown("""
    ...
    .btn-copy { 
        background-color: #f0f0f0 !important; color: #333 !important; padding: 12px; border-radius: 10px; 
        cursor: pointer; text-align: center; font-weight: 900; border: 1px solid #ccc; width: 100%;
...
with col_r:
    ...
    if st.session_state.res_list:
        # 使用 JSON 字符串转义，确保数据量再大也能完美写入剪贴板
        all_text = " ".join(st.session_state.res_list)
        st.markdown(f"""
        <div class="btn-copy" id="copy-btn" onclick="
